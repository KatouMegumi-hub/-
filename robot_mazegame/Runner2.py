...
class Runner(object):
    ...
    def run_training(self, training_epoch, training_per_epoch=150, display_direction=False):
        ...
        def train_logger_before_act(e, i):
            ...
            # 记录target_Qtable
            self.train_robot_record[(e, i)]['target_qtable'] = self.robot.target_Qtable[self.robot.sense_state()].copy()
            ...
            #修改应该记录的是target_Qtable
            if self.display_direction:
                self.train_robot_record[(e, i)]['target_qtable'] = deepcopy(self.robot.target_Qtable)

        def train_logger_after_act(e, i, action, reward):
            self.train_robot_record[(e, i)]['action'] = action  # 应该在这里记录action
            self.train_robot_record[(e, i)]['reward'] = reward  # 应该在这里记录reward
    ...
    def draw_robot(self, base_image, parameters):
        ...
        if self.display_direction:
            for state, q in parameters['target_qtable'].items():
                r, c = state
                direction = max(q, key=q.get)
                img[r * grid_size:(r + 1) * grid_size, c * grid_size:(c + 1) * grid_size, :] = \
                    (0.3 * self.arrow_img[direction] + 0.7 * img[r * grid_size:(r + 1) * grid_size,
                                                             c * grid_size:(c + 1) * grid_size, :]).astype('uint8')
    ...
    def write_on_header(self, header_img, parameters):
        ...
        # Flatten target Q-table for display
        target_q_values = '\n'.join([f"{state}: " + ', '.join([f"{a}: {q:.2f}" for a, q in actions.items()])
                                     for state, actions in parameters['target_qtable'].items()])
        line2 = "Target Q-values:\n" + target_q_values

        if not parameters['success']:
            line3 = "\nAction %s, Reward received %f \n" % (parameters['action'], parameters['reward'])
            line4 = "Epsilon: %.2f, Alpha: %.2f, Gamma: %.2f" % (
            parameters['epsilon'], parameters['alpha'], parameters['gamma'])
        else:
            line3, line4 = '', ''
    ...
    def plot_results(self):
        ...
        plt.subplot(133)
        plt.title("Running Times per Epoch")
